...
# 실패: 바닥에 어떤 정해진 높이보다 버블이 낮게 내려오면 성공
...
class Pointer(pygame.sprite.Sprite):
    def __init__(self, image, position, angle):
        ...
        self.rect = image.get_rect(center=position)
        self.angle = angle
        self.original_image = image # 원래 이미지에서 각도만큼 움직여야 하므로, original을 설정
        self.position = position

    def draw(self, screen):
        screen.blit(self.image, self.rect)

    def rotate(self, angle): # 각도 왔다갔다 함수
        self.angle += angle
...
def get_bubble_position(row_idx, col_idx): # 버블 포지션 정의
...
def get_bubble_image(color): # 버블 색깔 정의
    if color == "R":
...
def prepare_bubbles(): # 버블이 날라가면 새로 준비해주는 함수
...
def process_collision(): # 충돌 처리 함수
    ...
    if hit_bubble or curr_bubble.rect.top <= wall_height:
        row_idx, col_idx = get_map_index(*curr_bubble.rect.center) # *사용 이유: 튜플로 구성된 매개변수를 unpacking하여 각각의 변수로 나눠줌
...
def get_map_index(x, y): # 2차원 배열 좌표 get 함수
...
def place_bubble(bubble, row_idx, col_idx): # bubble group에 bubble add하는 함수
...
def remove_adjacent_bubbles(row_idx, col_idx, color): # 탐색을 통해 bubble remove 함수
    visited.clear()
...
def visit(row_idx, col_idx, color=None): #dfs를 통한 탐색 함수
    # 맵의 범위를 벗어나는지 확인
...
def remove_visited_bubbles(): # bubble 닿았을 때 버블 탐색 
...
def remove_not_visited_bubbles(): # 천장에서 떨어져 있는 버블 탐색
    bubbles_to_remove = [b for b in bubble_group if (b.row_idx, b.col_idx) not in visited]
    for bubble in bubbles_to_remove:
        map[bubble.row_idx][bubble.col_idx] = "."
        bubble_group.remove(bubble)

def remove_hanging_bubbles(): # 천장에서 떨어져 있는 버블 삭제
    visited.clear()
...
def drop_wall(): # 위에서 내려오는 벽 그리는 함수
...
def get_lowest_bubble_bottom(): # 가장 아래의 버블 rect값 get 함수
    bubble_bottoms = [bubble.rect.bottom for bubble in bubble_group]
    return max(bubble_bottoms)

def change_bubble_image(image): # 버블 색 검정색으로 바꾸는 함수
    for bubble in bubble_group:
        bubble.image = image

def display_game_over(): # 게임 종료 호출 함수
...
pygame.display.set_caption("Bubble Bubble") 
...
while running:
    ...
    screen.blit(wall, (0, 0 - screen_height + wall_height ))
